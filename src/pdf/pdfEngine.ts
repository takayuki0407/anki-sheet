 ...
 * Reuses the real detection so "what counts as an answer" is identical to the full pass. Falls back
 * to the magenta default when nothing colored is found (the user can still change the color and
 * re-detect afterwards). Cheap — one render per sample page, four classification passes per render.
 */
  ...
  try {
    ...
    try {
      for (const pi of pages) {
        ...
        try {
          const c = await renderPage(page, scale, canvas, undefined, true);
          const px = pixelsFrom(c);
          const runs = await runCandidates(page, scale);
    ...
    // Require a small floor so a stray false-positive doesn't pick a wrong color.
