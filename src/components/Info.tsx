...
const FAQ = [
  {
    ...
    a: "色付きの答え（赤・マゼンタなど）が印刷されたPDFを取り込むと、答えの部分を自動で検出して隠せます。タップで答えを確認しながら暗記でき、隠し方は『赤マスク』（答えを個別に隠す）と『赤シート』（半透明のシートをスライド）から選べます。PDFの解析は端末内で完結します（解析のために送信されることはありません）。クラウド同期（Pro・Premium）を使う場合のみ、ご自身の端末間で共有するためにPDFがアカウントに保存されます。",
  },
  {
    q: "PDFの取り込み方",
    a: "本棚の「＋ 取り込む」からファイルを選びます。答えの色は『自動検出（おまかせ）』で自動判定できるほか、赤／マゼンタ／橙／青のプリセット選択や微調整も可能です。検出には少し時間がかかります（中止可）。スキャン画像のPDF（テキスト層が無いもの）は現在未対応です（今後のアップデートで対応予定）。",
  },
  {
    q: "検出がうまくいかない時",
  ...
  {
    q: "AIで問題を作る（○×・4択）",
    a: "本の「問題」から、ページの本文と赤シートで隠す語句をもとにAIが○×問題・4択問題を自動生成します。覚えたつもりを“解いて確かめる”ための機能です。この機能だけは、選んだページの本文と語句を当アプリのサーバー経由で外部のAIサービスに送信します（初回に同意確認）。送信した内容は既定でAIモデルの学習には使われません。生成される問題は誤りを含む場合があるため、内容はご自身で確認してください。色の検出・赤シートなど他の機能は端末内だけで完結します。生成した問題はご自身のアカウント内に保存され、ほかのユーザーへ共有されません。プラン別に月間の生成回数の上限があります（1回＝1ページ×問題の種類。現在のプランは『情報・ヘルプ → プラン』で確認）。",
  },
  {
    q: "プランについて",
    a: "サインインだけで使える Free（本1冊・AI生成 月1回）／Standard ¥300（本10冊・AI 月10回）／Pro ¥600（本 無制限・AI 月30回・全端末でクラウド同期）／Premium ¥980（Proの全機能・AI 月100回・「今日の復習」で間違えやすい問題を最適なタイミングに再出題。初回7日間無料）。AI生成は「1回＝1ページ×問題の種類」で数えます。本の冊数はアカウント全体（すべての端末の合計）で数えます。未契約でもロックされず Free として使えます。料金は「料金プラン」ページ、現在のプランは「情報・ヘルプ → プラン」で確認できます。",
  },
  {
    q: "本のクラウド保存と復元（Pro・Premium）",
    a: "Pro・Premium で取り込んだ本は PDF・検出結果・学習の進捗（★・しおり・読書位置）がクラウドに保存され、別の端末からも取り込み直せます。Standard/Free に下げると上限を超えた本はクラウドに退避し（約6ヶ月保持）、再び Pro・Premium にすると復元できます。本棚の各本の「☁️ クラウドあり／端末のみ」表示で、削除しても後で戻せるかが分かります（「端末のみ」は削除すると復元できません）。",
  },
  {
    q: "ビューアの操作",
    a: "答えをタップで表示／もう一度タップで再び隠す。隠し方は『赤マスク』（答えを個別に隠す）と『赤シート』（縦読みで半透明シートをスライド）から選べます。倍率は ± と数値入力、「100%」ボタンで等倍に戻せます。『縦読み／横読み』『目次』『★（覚えにくい答えだけ復習）』にも対応。目次の各項目は ✎ で名前を編集できます。",
  },
];
        ...
        <div className="info-card">
          {user ? (
              ...
              <div className="danger-zone">
                {!delOpen ? (
                  ...
                  <div className="delete-account">
                    <p className="small">
                      アカウントを削除すると、
                      <strong>クラウドに保存されたPDF・検出結果・進捗、およびこのブラウザ内のデータも
                      すべて削除</strong>され、元に戻せません。確認のためパスワードを入力してください。
                    </p>
                    <input
                      type="password"
                      className="del-pw"
            ...
            <>
              ...
              <p className="muted small">
                本アプリのご利用には無料のアカウント登録（ログイン）が必要です。取り込んだPDF・検出結果・
                目次は<strong>このブラウザ内</strong>に保存され、クラウド同期対応プラン（Pro・Premium）
                でのみ端末・プラットフォーム間で同期されます。
              </p>
