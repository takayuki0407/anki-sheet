...
const FAQ = [
  ...
  {
    q: "AIで○×問題を作る（解いて確かめる）",
    a: "本の「問題」から、ページの本文と赤シートで隠す語句をもとにAIが正誤問題（○×）を自動生成します。覚えたつもりを“解いて確かめる”ための機能です。この機能だけは、選んだページの本文と語句を当アプリのサーバー経由でAI（Anthropic）に送信します（初回に同意確認）。色の検出・赤シートなど他の機能は端末内だけで完結します。生成した問題はご自身のアカウント内に保存され、ほかのユーザーへ共有されません。プラン別に月間の生成ページ数の上限があります（残り枠は『情報・ヘルプ → プラン』で確認）。",
  },
  {
    q: "プランについて",
