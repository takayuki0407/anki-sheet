...
色付きの答えを自動検出し、**デジタル赤シート**として通読できる **完全クライアントサイドの
Webアプリ** です。PDFはこの端末のブラウザ内だけで処理され、どこにもアップロードされません。

- **赤シート・ビューア（「めくる」）**: ページをそのまま描画し、答えを赤シートで一括ON/OFF、
  個別タップで部分表示。物理の赤シートそのままの使い心地です（CIDフォントのPDFでも動作）。
- **ページ移動**: 前後送り・ページ番号入力・スライダーで素早く移動。
