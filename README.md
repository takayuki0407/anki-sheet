...
- **AI問題生成（○×・4択）**: ページの本文と暗記語句から、AIが**○×（正誤）と4択**の問題を自動生成（種類は生成時に選択。同じページでも別々に生成・管理）。前後ページの文脈も参考に渡し、ページ境界で切れた語句も出題できます。生成中は進捗バー＋ページごとの状態表示（キャンセル・失敗まとめて再試行・完了サマリ→演習へ）。この機能のみ、選んだページの本文と語句を当アプリのサーバー経由で**外部のAIサービス**に送信します（初回に同意確認）。**生成内容は誤りを含む場合があり**、送信内容は**既定でAIモデルの学習には使われません**。
- **問題一覧・演習**: 生成した問題はページ×種類のグループで閲覧・再生成・削除。演習は**種類（○×/4択/ミックス）×範囲（全ページ/ページ範囲）×出題**を選んで開始。すべての解答の正誤を端末に記録し、**「間違えた問題だけ」復習は全プラン**で使えます。
- **今日の復習（Premium・SM-2）**: 解答の正誤から SM-2 で次回出題日を計算し、期限が来た問題だけを全ブック横断で出題（本棚のカードから）。復習状態は Premium で端末間同期（per-key LWW）。
...
src/sync/     バックエンド同期クライアント（books 登録 / R2 / 進捗 / reviews / エラー文言）。
              srs.ts = SM-2＋reviews LWW（iOS とバイト同一・check:shared 対象）
src/ai/       AI問題生成クライアント（qtype・前後文脈・クラウド復元）＋初回同意
src/components/  DeckList / ImportWizard / PageViewer / Settings / Info /
                QuizScreen（生成・問題一覧・演習）/ SolveSession / ReviewScreen（今日の復習）/
...
- **D1**（`anki-sheet-db`）: `users`（`tier` / `trial_until` / `downgraded_at`）/ `books`（アカウントの本の登録）/
  `progress` / `questions`（AI問題・`qtype`/`choices`）/ `reviews`（SM-2 状態・per-key LWW）/
  `generation_usage`（月間生成枠）。スキーマは `migrations/`。
...
- **AI問題生成**（`functions/api/sync/generate.ts`）: 選択ページの本文＋暗記語句（＋前後ページの参考文脈）を
  受け取り、**Claude Haiku 4.5**（`claude-haiku-4-5-20251001`）で **○×（`qtype=tf`）／4択（`qtype=mc4`）** を生成。
  枠・キャッシュ・REPLACE はすべて（ページ×種類）単位。API を呼ぶたび（初回・再生成とも）枠を原子的に
  予約し失敗時は返金、無料なのはキャッシュ再表示のみ（`functions/_lib/tier.ts` の `genPageLimit` /
  `genLimitDuringTrial`）。`ANTHROPIC_API_KEY` 未設定時は 503（`ai_not_configured`）。
- **SM-2 復習同期**（`functions/api/sync/reviews.ts`）: 解答成績（reviews）の pull / batch upsert。
  **書き込みは Premium のみ**（SQL 内 per-key LWW: `excluded.updated_at > reviews.updated_at`）。
  再生成・本/アカウント削除時に孤児 reviews も同時削除。
