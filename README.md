...
- **サインイン必須**（Firebase Auth・メール/パスワード）。アプリ画面はログインしないと使えません。
- **プラン**（7 日間の無料トライアルあり・無料プランなし）:

  | プラン | 月額 | 取り込み冊数 | クラウド同期 |
  |---|---|---|---|
  | **Standard** | ¥300 | **端末ごと 10 冊** | なし |
  | **Pro** | ¥600 | 無制限 | あり（R2・合計 5GB / 1ファイル 100MB まで） |

  - 開発者アカウント（管理者メール）は常に無制限。
- **端末ごとの上限（重要な設計）**: Standard の 10 冊は **その端末のローカル冊数** で判定します
  （アカウント全体の合計ではありません）。サーバはアカウントの総数で制限しないため、N 台なら 10×N 冊まで持てます。
- **クラウド同期（Pro）**: PDF 実体・デッキ内容（色設定・検出結果・しおり）・進捗（位置・表示済み・★・しおり）を
  端末間で同期します（**後勝ち = Last-Write-Wins**）。同期は常に **fail-open**（失敗してもローカル動作は止めません）。

### 強制モデル（「無料での継続利用」「プラン超過」を起こさない）
...
| ダウングレード（Pro→Standard）で端末が上限超過 | **強制トリム画面**で残す 10 冊を選び、他はローカル削除。クラウド(Pro)コピーは**保持**し、再 Pro で復元可能 |
| 取り込みで上限到達 | 取り込みをブロック（不要な本を削除 or Pro 案内） |
| 6 ヶ月リテンション | 非 Pro が 6 ヶ月続いたアカウントの保持クラウドデータを、日次 cron Worker が削除（`users.downgraded_at` 起点） |
...
- **RevenueCat webhook**（`functions/api/webhook/revenuecat.ts`）: 課金イベントで `users.tier` と
  `downgraded_at`（Pro で NULL、非 Pro で最初の降格時刻を保持）を更新。
